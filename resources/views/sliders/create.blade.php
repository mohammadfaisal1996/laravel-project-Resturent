                    ...
                    <form method="post" action="{{route("sliders.store")}}" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6 selects" id="categoires" style="@if($errors->has('category')) display:block @endif">
                                    <div class="form-group">
                                        <label for="exampleSelect1" >Category</label>
                                        <select class="form-control @if($errors->has('category')) is-invalid @endif" id="NavigateList" name="category">
                                            <option value="">None</option>
                                            @foreach($categories as $category)
                                                <option value="{{$category->id}}" {{selected("navigate_id", $category->id)}}>{{$category->category_name_en}} ( {{$category->category_name_ar}} )</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error("category")
                                ...
                                <div class="col-lg-6 selects" id="items" style="@if($errors->has('item')) display:block @endif">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @if($errors->has('item')) is-invalid @endif" id="NavigateList" name="item">
                                            ...
                                            @foreach($items as $item)
                                                <option value="{{$item->id}}" {{selected("navigate_id", $item->id)}}>{{$item->item_name_en}} ( {{$item->item_name_ar}} )</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error("item")
